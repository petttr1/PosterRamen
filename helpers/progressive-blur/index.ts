...
const getApplicableStylesForStep = (
  step: number,
  steps: number[],
  blurAmount: number,
) => {
...
const getBlurWrapperStyle = (detail: number, scale: number) => {
  return {
    ".progressive-blur": getStyleString({
      position: "absolute",
      "z-index": `${detail + 1}`,
      width: "100%",
      height: "100%",
...
const getBlurDisplayStyles = (
  ...
  for (const step of getFilterSteps(detail)) {
    ...
    result[`.progressive-blur > div:nth-child(${step + 1})`] =
      getStyleString(cssObject);
  }
  return result;
};
